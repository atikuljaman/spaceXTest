...
import "./UnsuccessfulLaunch.css";

const UnsuccessfulLaunch = () => {
  const [unsuccessfulLaunch, setUnsuccessfulLaunch] = useState([]);
  ...
  useEffect(() => {
    fetch(
      ...
      .then((res) => res.json())
      .then((data) => setUnsuccessfulLaunch(data));
  }, []);

  return (
    <div className="card-container">
      {unsuccessfulLaunch.length === 0 ? (
        <div className="spinner-container">
          <Spinner />
        </div>
      ) : (
        unsuccessfulLaunch?.map((data) => (
...
export default UnsuccessfulLaunch;
